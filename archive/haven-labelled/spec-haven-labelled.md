...
DESCRIPTION               # + labelled (>= 2.12.0) under Suggests
NEWS.md                   # four user-visible entries
man/                      # regenerated by devtools::document()
```

`NAMESPACE` does not change. `survey_data()` is already exported
(`NAMESPACE:106`) and gains an argument, not an export.

`DESCRIPTION` gains one `Suggests` entry and nothing else:
`labelled (>= 2.12.0)`. PR 4 adds it together with test row D-19
(`tests/testthat/test-utils.R:1004-1009`), which calls
`labelled::to_factor()` under `skip_if_not_installed("labelled")`. An
undeclared suggested package is an `R CMD check` note. `Imports` does not
change and `haven` stays in `Suggests`. Corrected 2026-09-03 (issue #216):
the first draft said `DESCRIPTION` does not change either.
...
`plans/error-messages.md` gains exactly one row and loses none, counted over
the feature's own commit range (`0be2f3a` to `14a2999`; see gate 13 for why
the base matters). The new row validates the `haven_class` flag from §VIII.1
rule 7:
...
Objectively verifiable. Gates 1 to 16 must hold; gates 17 and 18 are
recorded measurements, not pass-or-fail checks.

Three gates were reworded against measurement after the feature shipped,
because the form first written had never held: gates 2 and 7 on 2026-09-01
(#202), and gate 16 on 2026-09-03 (issue #216). Gate 13's count was rebased
on the feature's own commit range on the same date. Each carries the note
inline.
...
13. `plans/error-messages.md` rows PC-1, PC-2 and PC-3 describe the
    ...
    removed. The count runs over the feature's own commit range:
    `git diff 0be2f3a 14a2999 -- plans/error-messages.md`. `0be2f3a` is the
    base of the first feature PR and `14a2999` is the archive commit.
    Measured 2026-09-03: one row added (row 102, the D6 row), PC-1 to PC-3
    reworded, none removed. The count was first written against `cf6f153`,
    the commit the spec was drafted on, and cannot pass there — #185 merged
    between `cf6f153` and `0be2f3a` and added rows M-16 and M-17. Both ends
    of the range must stay fixed: #220 added row PC-15 after the archive
    commit. Rebased on the feature range 2026-09-03 (issue #216).
...
16. `NAMESPACE` is unchanged — `survey_data()` is already exported and gains
    an argument, not an export. `DESCRIPTION` gains one `Suggests` entry and
    nothing else: `labelled (>= 2.12.0)`, added by PR 4 for test row D-19
    (`tests/testthat/test-utils.R:1004-1009`), which calls
    `labelled::to_factor()` under `skip_if_not_installed("labelled")`. No
    `Imports` change and no version bump. Measured 2026-09-03 over
    `git diff 0be2f3a 14a2999`: `NAMESPACE` empty, `DESCRIPTION` one added
    line. **The "`DESCRIPTION` is unchanged" form never held.** The
    dependency is correct — an undeclared suggested package is an
    `R CMD check` note waiting to happen — so the gate was what was wrong,
    not the diff. Reworded 2026-09-03 (issue #216) rather than left recorded
    as met. A diff against `cf6f153` also shows two `NAMESPACE` exports from
    #185; see the base-commit note in gate 13.
17. The implementation notes record the timing ratio required by §III.6.
    ...
    No `implementation.md` was written for PR 3a, so the ratio went
    unrecorded until issue #216. Measured 2026-09-03 on `develop`, R 4.6.1,
    S7 0.2.2. One grouped call —
    `get_quantiles(des, age, quantiles = c(0.25, 0.5, 0.75), group = pid3)`
    — on `ns_wave1`, the widest bundled dataset at 171 columns and 6,422
    rows, with a design from `as_survey_nonprob()`. 30 calls per block,
    three alternating blocks, minimum taken:
    ...
    The ratio is below 1.00 and inside the run-to-run spread of about 5%, so
    the strip has no measurable cost at this width. §III.6 said to stop and
    report above about 1.05. Two supporting figures, on a 500-column,
    200-row frame where no column carries the class: 0.45 ms per
    `.strip_labelled_columns()` call, and 1.0 to 1.25 ms per
    `des@data <- df` assignment. Recorded 2026-09-03 (issue #216).
18. The implementation notes record the S7 version the setter behaviour in
    §III.4 was confirmed against.

    Also unrecorded, for the same reason. Confirmed 2026-09-03 against
    **S7 0.2.2** on R 4.6.1: one `des@data <- df` assignment enters
    `.strip_labelled_columns()` exactly once, traced on the helper. The
    setter does not re-enter itself, so the §III.4 comment in
    `R/core-classes.R:319-320` holds on this version. Recorded 2026-09-03
    (issue #216).

Gates 17 and 18 had no record because PR 3a's directory under
`.surveycore-workspace/` is empty, and `.surveycore-workspace/` is
untracked. PRs 5 and 7 wrote notes; PRs 1, 2, 3a, 3b, 4, 6 and 8 wrote none.
Issue #217 owns the open question of which pipeline artifacts get archived
with a feature, `implementation.md` included. Until that is settled, this
section is the durable record for both gates.
