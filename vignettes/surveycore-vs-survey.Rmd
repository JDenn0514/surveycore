...
to new users. `as_survey_srs()` names the design type explicitly. When neither
`ids` nor `strata` are specified, `as_survey()` also dispatches to
`as_survey_srs()` automatically.
...
`as_survey_calibrated()` preserves the distinction in code, output, and
...
```{r surveytidy-load, eval=has_surveytidy, include=FALSE}
library(surveytidy)
```
...
| **Non-probability design** | No dedicated constructor | No dedicated constructor | `as_survey_calibrated()` |
