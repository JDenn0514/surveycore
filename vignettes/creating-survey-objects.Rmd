...
| Comes with pre-built replicate weight columns (repwt_1, repwt_2, …) | `as_survey_repweights()`        | Uses the agency-supplied variance replicates     |
...
| ACS PUMS (1-year)         | `as_survey_repweights()`        | 80 successive-difference replicate weights    |
| Pew Jewish Americans 2020 | `as_survey_repweights()`        | 100 JK1 jackknife replicate weights           |
| BRFSS                     | `as_survey_repweights()`        | Bootstrap replicate weights                   |
| NAEP / PISA               | `as_survey_repweights()`        | JK2 jackknife replicate weights               |
| Nationscape (Democracy Fund + UCLA) | `as_survey_calibrated()` | Non-probability quota panel; ACS-calibrated raking weights |
...
## 3. `as_survey_repweights()` — Replicate Weight Designs {#sec-rep}

Use `as_survey_repweights()` when your data provider has supplied pre-computed
...
textbook use case for `as_survey_calibrated()`.
...
the sampling protocol. Using `as_survey_calibrated()` makes this explicit
...
| | `as_survey()` / `as_survey_repweights()` | `as_survey_srs()` | `as_survey_calibrated()` |
|---|---|---|---|
...
|---|---|---|
| Probability sample with design weights | `as_survey()`, `as_survey_repweights()`, `as_survey_srs()` | Exact variance |
...
| "base weight", "design weight" (with separate replicates)        | `weights =` in `as_survey_repweights()`     |                                           |
