# R/14-analysis-ratios.R
#
# Survey-weighted ratio estimation using the delta method (Taylor/SRS/
# calibrated/twophase) or direct per-replicate computation (replicate).
# Exported functions: get_ratios()
#
# Statistical method:
#   ratio = Σ(wᵢ yᵢ) / Σ(wᵢ xᵢ)   [numerator total / denominator total]
#
#   Taylor / SRS / calibrated / twophase:
#     Delta method — linearize as z_i = y_i - ratio * x_i, inject into
#     design@data, compute SE of total(z) via .total_cell(), divide by
#     |total_x|. Equivalent to survey::svyratio() for these designs.
#
#   Replicate designs:
#     Compute ratio_r = total_y_r / total_x_r for each replicate, then
#     compute variance via .svy_rep_var(). Matches survey::svyratio() exactly
#     (the delta-method linearization produces a different SE for replicate
#     designs because it approximates each replicate ratio via linearization
...
get_ratios <- function(
  ...
  if (length(num_names) != 1L) {
    cli::cli_abort(
      c(
        "x" = "{.fn get_ratios} requires exactly one {.arg numerator} variable.",
  ...
  for (ci in seq_len(n_combos)) {
    ...
    if (is_replicate) {
      se_result <- .replicate_ratio_cell(
        design, y_col, x_col, domain_vec, ratio_est, total_x
      )
    } else {
      se_result <- .delta_ratio_cell(
        design, y_col, x_col, domain_vec, ratio_est, total_x, tmp_col
      )
    }
