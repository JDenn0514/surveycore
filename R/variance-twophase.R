...
.twophase_phase1_var <- function(influence, design, method, lonely.psu) {
  ...
  if (identical(method, "simple")) {
    ...
    psu_per_stratum <- tapply(psu_all, strata_all, function(ps) length(unique(ps)))
    ...
    v    <- .svy_recvar(matrix(score, ncol = 1L), clusters_mat, strata_mat, fpcs,
...
.twophase_phase2_var <- function(influence, design, subset, lonely.psu) {
  ...
  psu2_per_stratum <- tapply(psu2_col, strata2_col, function(ps) length(unique(ps)))
  ...
  popsize2_mat <- if (!is.null(ph2_vars$fpc)) {
    ...
    pop2_per_stratum <- tapply(psu2_all, strata2_all,
  ...
  v <- .svy_recvar(score_mat, clusters2_mat, strata2_mat, fpcs2,
                   lonely.psu = lonely.psu)
  v[1L, 1L]
}
...
.twophase_build_inputs <- function(design, y_col, na.rm) {
  ...
  # Calibrated weights: Phase 1 weight divided by conditional Phase 2 probability
...
.twophase_df <- function(design) {
  ...
  psu_per_stratum <- tapply(psu_col, strata_col,
                            function(p) length(unique(p)))
