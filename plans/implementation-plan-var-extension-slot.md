...
- [ ] PR 1: `feature/var-extension-slot` — add the `var_extra` metadata
  ...
  **Quality gates**
  ...
  - `tests/testthat/test-metadata-system.R` — modified (all
    ...
    documentation).
  - `tests/testthat/test-validators.R` — modified (rename/delete
    lifecycle tests for `var_extra`), unless the package's existing
    equivalent `notes`-rename tests live elsewhere, in which case mirror
    that file instead (confirm during task 76).
