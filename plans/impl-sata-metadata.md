...
- [ ] PR 2: `feature/extract-tidyselect` — Migrate all seven `extract_*()` functions from `.resolve_vars()` to `tidyselect::eval_select()`
- [ ] PR 3: `feature/set-extract-sata` — Implement `set_sata()` and `extract_sata()` with `.format_logical_result()`
