...
| `R/core-metadata.R` | Replace/update all setter and extractor functions; add `extract_metadata()`; add `.check_is_survey_or_df()`, `.resolve_vars_from_x()`, `.parse_setter_input()`, `.format_scalar_result()`, `.format_list_result()` internal helpers |
...
The old `.check_is_survey()` is retained for functions that intentionally do
NOT accept data frames (none in this spec — all functions gain data frame
support). If `.check_is_survey()` is still referenced elsewhere in the
codebase, it is not removed; it is simply no longer the primary input check
here.
...
Shared parsing logic for all unified setters. Implements the three calling
convention detection logic described in Section IV. Returns a named list
mapping variable names to content values. Errors with
`surveycore_error_setter_ambiguous` if both `...` and `variable`/content are
provided; errors with `surveycore_error_setter_empty` if neither is provided;
errors with `surveycore_error_setter_mismatched_lengths` if `variable` and
content have different lengths (convention 3).
...
#### `.resolve_vars(x, var_exprs, all_cols, call)`

Resolves the `var` argument for extractors. If `var` is missing (the quosure
list has length 0), returns all column names from `all_cols`. Otherwise,
evaluates the expressions as bare column names or character vectors. Issues
`surveycore_warning_var_not_found` for any specified variable not found in
`all_cols` and returns only the valid names.

```r
.resolve_vars <- function(
  x,         # survey object or data.frame (used for data-masking context)
  var_exprs, # list of quosures from ...
  all_cols,  # character vector of available column names
  call = rlang::caller_env()
)
```
...
Note: `extract_metadata()` does NOT have `format` or `fill` arguments.
Variables are always included regardless of whether they have metadata (all
fields default to `NULL` for unset metadata). This provides a complete
inventory of the design's variables, not just those with metadata.
...
**For variables with no metadata set in any field:** All fields are `NULL`
(except `transformations` which is always `list()`). The variable is still
...
- All variables specified (or all columns if `...` omitted) are returned,
  whether or not they have any metadata.
...
d <- set_var_label(d, ridageyr = "Age at screening", riagendr = "Gender")
d <- set_val_labels(d, riagendr = c(Male = 1L, Female = 2L))
d <- set_universe(d, ridageyr = "All participants")
d <- set_missing_codes(d, ridageyr = c("Refused" = 999L))

extract_metadata(d, ridageyr, riagendr)
#> $ridageyr
...
The `survey_metadata()` constructor call at the end of `.extract_haven_metadata()`
must pass the two new fields.
...
extract_var_label(svy, age)    # "Age in years"
extract_var_label(svy, sex)    # "Sex of respondent"
extract_val_labels(svy, sex)   # c(Male = 1L, Female = 2L)
extract_universe(svy, age)     # "All adults 18+"
extract_missing_codes(svy, sex) # c("Refused" = 9L)
```
...
**Detection and error message:** When `.parse_setter_input()` receives a
`dots` list with exactly two elements where the first element is an unquoted
symbol (detected via `rlang::is_symbol(rlang::quo_get_expr(dots[[1L]]))`) and
the second is a scalar string with no name, issue a targeted error with class
`surveycore_error_old_positional_setter`:
...
**NEWS.md entry:** This breaking change must be documented in a `### Breaking
Changes` section of NEWS.md for the release that includes this spec.
...
- Variable skipped but other variables still set
...
- Round-trip: set on df → `as_survey()` → extract from survey object (one block per field)
...
**Happy path — single variable (backward compatibility):**
- `extract_var_label(svy, age)` returns the label string (or `NULL`)
- Behavior identical to the old API for single-variable calls
...
- Single variable with no fields set: returns 7-key list with all `NULL` (except `transformations = list()`)
- Multiple variables: returns one entry per variable
- No `var` argument: returns one entry per column in `x@data`
- `transformations` is always `list()`, never `NULL`
- `weighting_history` is NOT present in any entry of the result

**Edge cases:**
- All variables with no metadata: returns list of all-NULL entries
- Single-column data frame: returns list with one entry
