...
**No new error class.** An earlier draft added one for "`haven` is not
installed", because the rebuild called `haven::labelled()`. The rebuild uses
base R instead (§II), so that condition cannot arise and the class is not
needed. `plans/error-messages.md` gains no row from `survey_data()`.
...
7. `haven_class` must be a length-one logical. A non-logical or
   ...
   the package validates other scalar flags and reuse the existing class.
...
### IX.1a No new class, and none removed

`plans/error-messages.md` gains no row and loses none. Every class this spec
names already exists there. Two drafts proposed additions and both were
withdrawn:
...
The only edits to `plans/error-messages.md` are the three Condition-column
corrections in §VI.8.
...
13. `plans/error-messages.md` rows PC-1, PC-2 and PC-3 describe the
    conditions the code now raises on. No row added, no row removed.
