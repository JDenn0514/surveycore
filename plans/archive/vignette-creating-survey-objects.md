...
- Three branches:
  1. "My data has explicit replicate weights (repwt_1, repwt_2, ...)" → `as_survey_replicate()`
...
- Quick-reference table: common survey programs and which constructor they use
  ...
  | BRFSS | `as_survey_replicate()` | Provides pre-computed raking weights as replicates |
  | ACS PUMS | `as_survey_replicate()` | Provides 80 replicate weights (pwgtp1-pwgtp80) |
  | CPS ASEC | `as_survey_replicate()` | Provides replicate weights |
...
| "base weight", "design weight" | `weights =` in `as_survey_replicate()` | |
