...
| # | Function | Condition | Level | Error Class | cli Message Template |
|---|----------|-----------|-------|-------------|----------------------|
| PC-1 | `get_corr(method = "polychoric")` | One or more selected variables are not ordinal. Ordinal means `factor`, `ordered`, small-cardinality `integer`, or a `double` whose non-missing values are all finite and all whole, with no more than the cardinality cutoff distinct values | ERROR | `surveycore_error_polychoric_requires_ordinal` | `"x" = "{.code method = \"polychoric\"} requires ordinal variables. Non-ordinal column{?s}: {.field {bad_vars}} ({.cls {bad_classes}}).", "v" = "Coerce to {.cls factor} or {.cls ordered}, or use {.code method = \"pearson\"}."` |
