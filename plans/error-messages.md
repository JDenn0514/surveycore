...
| 88 | `survey_data()` | `haven_class` is not a length-one logical (e.g., `NA`, `NULL`, `"yes"`, `c(TRUE, TRUE)`) | ERROR | `surveycore_error_haven_class_not_logical` | `"x" = "{.arg haven_class} must be {.code TRUE} or {.code FALSE}.", "i" = "Got {.obj_type_friendly {haven_class}}."` |
